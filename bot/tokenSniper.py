...
bscNode = obj['bscNode'] #set the BSC node to use.

w3 = Web3(Web3.HTTPProvider(bscNode))

if w3.isConnected:
    print("[+] Connection Esatablished\n")

pancakeSwapRouterAddress = '0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D'   #uniswap_router
# pancakeSwapRouterAddress = obj['pancakeSwapRouterAddress'] #load config data from JSON file into program
pancakeSwapFactoryAddress = '0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f'  #uniswap_factory
...
def checkTokenPrice(tokenAddress):
    BNBTokenAddress = Web3.toChecksumAddress("0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2")  # WETH
    amountOut = None
...
def Sell(sellTokenContract, tokenValue, tokenReadableBal, tokenContractAddress):
    ...
    spend = web3.toChecksumAddress("0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2")  #weth contract address
...
def foundToken(event):
    try:
        ...
        if ((jsonEventContents['args']['token0'] == "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2") or (jsonEventContents['args']['token1'] == "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2")): 
        #check if pair is WBNB, if not then ignore it
        
            if (jsonEventContents['args']['token0'] == "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2"):
         ...
         #--------------------------------------------MINI AUDIT FEATURE-------------------------------------------------------
            ...
            if(enableMiniAudit == True): #enable mini audit feature: quickly scans token for potential features that make it a scam / honeypot / rugpull etc
                ...
                contractCodeGetRequestURL = "https://api.etherscan.io/api?module=contract&action=getsourcecode&address=" + tokenAddress + "&apikey=" + bscScanAPIKey
...
#-----------------------------------------TOKEN SCANNER MONITORING/SELL CALCULATION ----------------------------------------------------------------------
